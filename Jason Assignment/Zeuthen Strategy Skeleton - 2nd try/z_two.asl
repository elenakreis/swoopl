...
wrisk([MyD1, MyD2], MyWrisk, TheirWrisk) :-
	...
	MyWrisk = (U_MyD2 - U_TheirD2) / U_MyD2 & 
...
+!firstRound
	...
	.send(z_one, tell, theirDeal(BestDeal));
	+myDeal(BestDeal);
	!checkAccept.
...
+!checkAccept
	...
	<-
	// accept their deal
	.print("Agent 2 accepts");
	.send(z_one, tell, acceptedDeal(TheirDeal));
