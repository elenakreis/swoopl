:- [diagnosis].
%%% intersect(L1,L2,X) unifies X with the intersection between L1 and L2.
  intersect([], _, []).
  %If H1 is in L2
...
%%% getLabel(F, Hn, Label) unifies Label wit the first item in F for which holds that the intersection between Hn and S\in F == [].
